...
import { DevelopersDataTable, RepositoriesDataTable } from "../../components/data-tables";
...
export default async function Home({
  ...
  const { summary: s, repositories, developers, models, providers } = data;
  ...
  return (
    <Page>
      ...
      {developers.length ? (
        <DevelopersDataTable
          queryString={query.size ? `?${query}` : ""}
          rows={developers.map((developer: any) => ({
            id: String(developer.id),
            developer: developer.email
              ? String(developer.email)
              : `Identity ${String(developer.id).slice(0, 8)}`,
            provider: String(developer.provider),
            repositories: Array.isArray(developer.repositories)
              ...
              : [],
            prompts: Number(developer.prompts ?? 0),
            firstObservedAt: developer.first_observed_at
              ? new Date(developer.first_observed_at).toISOString()
              : null,
            lastObservedAt: developer.last_observed_at
              ? new Date(developer.last_observed_at).toISOString()
              : null,
          }))}
        />
      ) : (
