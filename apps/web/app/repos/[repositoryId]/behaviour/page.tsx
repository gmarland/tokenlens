...
export default async function Behaviour({
  params,
  searchParams,
}: PageProps<"/repos/[repositoryId]/behaviour">) {
  ...
  const correlationsByContext = correlations(
    data.prompts,
    (prompt: any) => Number(prompt.context_tokens),
    {
    ...
    <Page>
      <Toolbar>
        <Box>
          ...
          <Intro>Prompt-level context and traversal patterns.</Intro>
        </Box>
        <AnalysisFilters
          idPrefix="behaviour"
          initialProvider={provider}
          initialModel={model}
          providers={data.providers.map((item: any) => ({
            value: item.provider,
            label: item.provider,
          }))}
      ...
      {correlationsByContext.length ? (
        <Cards compact>
          {correlationsByContext.map((correlation) => (
            <Card variant="outlined" key={correlation.name}>
              <CardContent>
