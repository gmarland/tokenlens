...
export default async function Prompts({ params, searchParams }: any) {
  ...
  return <Page>
    ...
    <Intro>Prompt bodies are captured and attributed to the provider identity reported for the session. Search the full prompt bodies or use the column headings to sort the current data.</Intro>
    <Stack
      component="form"
      method="get"
    ...
    >
      ...
      {q.provider ? <input type="hidden" name="provider" value={q.provider} /> : null}
      <TextField
        defaultValue={search}
        fullWidth
      ...
      {search ? (
        <Button component={Link} href={`/repos/${id}/prompts${clearSearch.size ? `?${clearSearch}` : ""}`} variant="outlined">
          Clear
        </Button>
      ) : null}
    </Stack>
    ...
    }))} /> : <Panel sx={{ p: 0 }}><EmptyState>{search ? "No prompts match this search and the current filters." : "No prompts match these filters."}</EmptyState></Panel>}
  </Page>;
}
