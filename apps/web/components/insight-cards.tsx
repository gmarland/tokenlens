        ...
        >
          ...
          >
            ...
            {insight.recommendation.href ? (
              <Button
                component={Link}
                href={insight.recommendation.href}
                size="small"
                variant="outlined"
              >
                Investigate
          ...
          >
            <Box component="summary" sx={{ cursor: "pointer" }}>
              ...
              >
                ...
                <Typography
                  ...
                  sx={{ fontWeight: 800 }}
                >
                  {insight.recommendation.example.title}
                </Typography>
              </Box>
            </Box>
            <Box component="ol" sx={{ my: 1, pl: 2.5 }}>
              {insight.recommendation.example.steps.map((step) => (
                <Typography
                  component="li"
                  variant="body2"
                  key={step}
            ...
            {insight.recommendation.example.snippet ? (
              <Box
                component="pre"
              ...
              >
                {insight.recommendation.example.snippet.content}
              </Box>
            ) : null}
          </Box>
          <Box component="details" sx={{ mt: 1.5 }}>
              ...
              {insight.evidence.map((item) => (
                <Box key={item.label}>
                  ...
                  <Typography variant="body2" sx={{ fontWeight: 700 }}>
                    ...
                    {item.baseline != null
                      ? ` · baseline ${evidenceValue(item.baseline, item.unit)}`
                      : ""}
                  </Typography>
                </Box>
              ))}
            ...
            >
              ...
              {insight.coverage == null
                ? ""
                : ` · ${(insight.coverage * 100).toFixed(0)}% coverage`}
              {insight.comparisonPeriod ? ` · ${insight.comparisonPeriod}` : ""}
            </Typography>
            <Typography variant="body2" sx={{ mt: 1 }}>
              <strong>Validate:</strong> {insight.validation.text}
            </Typography>
